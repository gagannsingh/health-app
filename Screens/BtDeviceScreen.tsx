...
interface HealthKitLinkProps {}

interface HealthKitLinkState {
  isHealthKitLinked: boolean;
}

export default class HealthKitLink extends React.Component<HealthKitLinkProps, HealthKitLinkState> {
    private healthKit = new HealthKit(); // creating the healthkit 

    constructor(props: HealthKitLinkProps) {
        super(props);
        this.state = { isHealthKitLinked: false };
        this.linkToHealthKit = this.linkToHealthKit.bind(this);
    }
    ...
    async linkToHealthKit(): Promise<void> {
      try {
        await this.healthKit.link(); // link to HealthKit
        this.setState({ isHealthKitLinked: true });
        console.log("Linked to HealthKit");
      } catch (ex) {
        console.error(`Error linking to HealthKit: ${JSON.stringify(ex)}`);
      }
    }
    ...
    render(): React.ReactNode {
        return (
            <View style={{ marginTop: 40, marginBottom: 40, flex: 1 }}>
                <Button title="Link to HealthKit" onPress={this.linkToHealthKit} />
                {this.state.isHealthKitLinked && <Text>HealthKit Linked</Text>}
